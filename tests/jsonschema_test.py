...
def test_logical_branch_isolation(builder):
    ...
    schema = (
        builder.any_of()
            .add().object().prop("branch_a").string().end().end()
            .add().object().prop("branch_b").integer().end().end()
        .end()
        .build()
    )
...
def test_string_constraints(builder):
    builder.prop("email").string().min_length(5).max_length(50).pattern("@").format("email").end()
...
def test_array_constraints(builder):
    builder.prop("items").array().min_items(1).max_items(3).unique_items().string().end().end()
