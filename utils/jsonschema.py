...
class JSONSchemaBuilder:
            ...
            case list() as types:
                if not any(t in types for t in allowed_types):
                    raise SchemaValidationError(
                        f"'{method_name}' requires {allowed_types}, found {types}."
                    )
    ...
    def build(self) -> Dict[str, Any]:
        ...
        def visit(node: str):
            if node in temp_stack:
                return  # Handle/allow recursion in Draft 2020-12
    ...
    def array(self) -> JSONSchemaBuilder:
        ...
        return JSONSchemaBuilder(
            self._schema["items"], parent=self, root=self._root_ref
        )
    ...
    def definition(self, name: str) -> JSONSchemaBuilder:
        ...
        if "$defs" not in root_s:
            root_s["$defs"] = {}
        root_s["$defs"].setdefault(name, {})
        return JSONSchemaBuilder(
            root_s["$defs"][name], parent=self, root=self._root_ref
        )
    ...
    def any_of(self) -> LogicalContext:
        return self._logic("anyOf")

    def one_of(self) -> LogicalContext:
        return self._logic("oneOf")

    def all_of(self) -> LogicalContext:
        return self._logic("allOf")
    ...
    def integer(self) -> Self:
        self._schema["type"] = "integer"
        return self

    def number(self) -> Self:
        self._schema["type"] = "number"
        return self

    def boolean(self) -> Self:
        self._schema["type"] = "boolean"
        return self

    def const(self, value: Any) -> Self:
        self._schema["const"] = value
        return self
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name.startswith("x_"):
            ...
            def wrapper(value: Any) -> Self:
                return self.set(name.replace("_", "-"), value)
...
class LogicalContext:
    def __init__(
        ...
        parent: JSONSchemaBuilder,
        root: JSONSchemaBuilder,
    ) -> None:
